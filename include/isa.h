...
// Now we could have a status register, but we going to bend the rules a little bit.
// Just define the flags enum here and then create an array of this enum size.
// We shall assume the remaining bits are padded off ..., as long as this length
// does not exceed the size of a register, we can assume we abstracted correctly.
//
// meaning -> Flag State array: [1], but hardware should look like [1000 0000 0000 0000]
enum Flags {
	ZERO_FLAG,
