...
int main() {
  ...
  // We run for a max of 20 cycles just in case we accidentally created an
  // infinite loop
  for (int i = 0; i < 20; i++) {
    // Read the instruction we are about to execute just to see if it's HALT
    // uint16_t currentPC = ram.read(cpu.m_Registers[ISA::PC]); // Note: You
    // might need a getter for PC if it's strictly private
